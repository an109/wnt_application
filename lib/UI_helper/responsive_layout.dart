...
class Responsive {
  // Base design size (change this to your Figma/XD design width)
  static const double designWidth = 375;  // Standard mobile design width
  static const double designHeight = 812; // Reference only - not used for scaling
  ...
  // PIXEL-PERFECT width-based scaling (RECOMMENDED - use this for everything)
  static double px(BuildContext context, double designPixels) {
    return (designPixels / designWidth) * screenWidth(context);
  }

  // Height scaling using WIDTH as reference (maintains proportion across all devices)
  static double py(BuildContext context, double designPixels) {
    return (designPixels / designWidth) * screenWidth(context);
  }

  // Responsive font size (pixel-perfect, no clamping)
  static double sp(BuildContext context, double designPixels) {
    return (designPixels / designWidth) * screenWidth(context);
  }
...
// COMPLETE REPLACEMENT EXTENSION - Pixel Perfect (Width-Based)
extension ResponsiveExtension on BuildContext {
  // Screen dimensions
  double get screenWidth => MediaQuery.of(this).size.width;
  double get screenHeight => MediaQuery.of(this).size.height;
  ...
  // ============ PIXEL-PERFECT SCALING METHODS (All use WIDTH as reference) ============

  /// Scale ANY dimension (width, height, padding, margin) from design pixels
  /// This maintains EXACT proportion across ALL devices
  /// Example: w(16) will be 16px on 375px screen, proportionally scaled on others
  double w(double designPixels) => (designPixels / _designWidth) * screenWidth;

  /// Scale height using width reference (keeps everything proportional)
  /// Use this for ALL heights, vertical paddings, vertical margins
  /// Example: h(24) maintains same proportion as w(24)
  double h(double designPixels) => (designPixels / _designWidth) * screenWidth;

  /// Scale font size - perfectly proportional, no clamping
  /// Fonts will scale exactly with screen width
  double fs(double designPixels) => (designPixels / _designWidth) * screenWidth;

  /// Scale border radius
  double r(double designPixels) => w(designPixels);

  /// Scale for square elements (uses width-based scaling)
  double sq(double designPixels) => w(designPixels);

  // ============ LEGACY SUPPORT (Keep for compatibility) ============

  // Old percentage-based methods (avoid using for new code)
  double wp(double percentage) => screenWidth * percentage / 100;
  double hp(double percentage) => screenHeight * percentage / 100;
  ...
  // ============ RESPONSIVE VALUES (All using w() for consistency) ============

  // Responsive padding
  EdgeInsets get responsivePadding {
    if (isMobile) return EdgeInsets.all(w(16));
    if (isTablet) return EdgeInsets.all(w(24));
    return EdgeInsets.all(w(32));
  }
  ...
  // Responsive gaps (all based on width)
  double get gapXXSmall => w(4);
  double get gapXSmall => w(6);
  double get gapSmall => w(8);
  double get gapMedium => w(12);
  ...
  // Font sizes (perfectly proportional)
  double get displayLarge => fs(57);
  double get displayMedium => fs(45);
  double get displaySmall => fs(36);
  double get headlineLarge => fs(32);
